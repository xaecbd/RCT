...
export default class StringTopKey extends Component {
  static displayName = 'StringTopKey';
  ...
  getData = () => {
    if (this.state.value.id && this.state.value.timeSelected) {
      axiosInstance
        .get(`/rdb/top_key?pid=${this.state.value.id}&scheduleId=${this.state.value.timeSelected}&type=0`)
        .then((response) => {
          if (response.data.code === 200 && response.data.data) {
            ...
            Toast.error('Get top key By String Error!');
          }
        })
        .catch((error) => {
          console.log(error);
