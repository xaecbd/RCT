...
export default class ListTopKey extends Component {
  static displayName = 'ListTopKey';
  ...
  getData = () => {
    if (this.state.value.id && this.state.value.timeSelected) {
      axiosInstance
        .get(`/rdb/top_key?pid=${this.state.value.id}&scheduleId=${this.state.value.timeSelected}&type=10`)
        .then((response) => {
          if (response.data.code === 200 && response.data.data) {
            this.setState({ total: response.data.data.length, totalData: response.data.data }, () => {
              console.log(`dataL${response.data.data}`);
            });
            this.getPageData(response.data.data);
          } else {
            Toast.error('Get Top Key By List Error!');
          }
        })
        .catch((error) => {
          console.log(error);
