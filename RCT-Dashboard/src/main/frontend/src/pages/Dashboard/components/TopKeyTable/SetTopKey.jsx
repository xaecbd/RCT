...
export default class SetTopKey extends Component {
  static displayName = 'SetTopKey';
  ...
  getData = () => {
    if (this.state.value.id && this.state.value.timeSelected) {
      axiosInstance
        .get(`/rdb/top_key?pid=${this.state.value.id}&scheduleId=${this.state.value.timeSelected}&type=15`)
        .then((response) => {
          if (response.data.code === 200 && response.data.data) {
            ...
            Toast.error('Get Top Key By Set Error!');
          }
        })
        .catch((error) => {
          console.log(error);
