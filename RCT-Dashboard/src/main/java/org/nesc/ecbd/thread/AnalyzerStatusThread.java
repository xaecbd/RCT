...
import org.nesc.ecbd.report.EmailSendReport;
...
public class AnalyzerStatusThread implements Runnable {
	...
	public AnalyzerStatusThread(List<AnalyzeInstance> analyzeInstances, RestTemplate restTemplate,
			RDBAnalyze rdbAnalyze, Email emailInfo, RDBAnalyzeResultService rdbAnalyzeResultService) {
	...
	public void run() {
		...
		if (AppCache.isAnalyzeComplete(rdbAnalyze)) {
			...
			try {
				Map<String, ReportData> latestPrefixData = rdbAnalyzeResultService.getReportDataLatest(rdbAnalyze.getPid());
			    try {
